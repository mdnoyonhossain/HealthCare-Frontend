...
import { useGetSinglePatientQuery, useUpdatePatientMutation } from "@/redux/api/patientApi";
...
const PatientProfileUpdateModal = ({ open, setOpen, id }: TUpdateModalProps) => {
    const [isLoading, setIsLoading] = useState(false);

    const { data: getSinglePatientData, refetch } = useGetSinglePatientQuery(id);
    ...
    const handleUpdateMyProfile = async (data: FieldValues) => {
        ...
        try {
            const res = await updatePatient({ body: updatedValues, id }).unwrap();
            if (res?.id) {
    ...
    return (
        <HCFullScreenModal open={open} setOpen={setOpen} title='Update My Profile'>
            <HCForm onSubmit={handleUpdateMyProfile} defaultValues={getSinglePatientData} resolver={zodResolver(UpdateMyProfilevalidationSchema)}>
