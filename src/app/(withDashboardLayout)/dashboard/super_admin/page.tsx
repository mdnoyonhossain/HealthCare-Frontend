...
const SuperAdminPage = () => {
    ...
    const stats = [
        { label: 'Admins', value: data?.adminCount, color: '#7e57c2' },
    ...
    const barChartData = {
        xAxis: [{
            ...
            data: data?.bartChartData?.map((d: any) =>
                new Date(d?.month).toLocaleDateString()
            ),
        }],
        series: [{
            label: 'Appointments',
    ...
    const pieChartData = data?.pieChartData?.map((item: any, index: number) => ({
        id: index,
        value: item.count,
        label: item.status
    }));
        ...
        >
            ...
            >
                ...
                <Typography variant="subtitle1" color="text.secondary">
                    Monitor appointments, payments, users, revenue and more.
                </Typography>
            </Paper>

            <Grid container spacing={3} mb={6}>
                {stats?.map((item, i) => (
                    <Grid sx={{ xs: 12, sm: 6, md: 4, lg: 2 }} key={i}>
                        <Paper
                            elevation={3}
                            sx={{
                                p: 3,
